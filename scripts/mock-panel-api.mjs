...
export const FIXTURES = {
  ...
  // B1.4: PROVIDER2's primary facility — multi-location fixture shape
  // (several assigned locations, one primary).
...
const PROVIDERS = [
  {
    ...
    groups: [
      { id: "g-1", name: "Northside Therapy Group", isPrimary: true },
      { id: "g-2", name: "Riverside PT", isPrimary: false },
    ],
  ...
  },
  {
    ...
    status: "active",
    groupId: "g-1",
    groups: [{ id: "g-1", name: "Northside Therapy Group", isPrimary: true }],
    specialty: "Physical Therapy",
...
// Pat Morgan (B1.4) carries the multi-location shape: several assigned
...
// "rich" shape (used by the <30-day expiry badge test). Pat Morgan (B1.4)
...
function profileTokens(providerId, selectedFacility, license) {
  ...
  return [
    ...
    { token: "user.email", value: "testkansas@example.com" },
  ];
}
...
export async function createMockPanelApi(options = {}) {
  ...
  async function handle(req, res) {
    ...
    if (/^\/api\/me\/orgs\/?$/.test(url.pathname)) {
      const rows = [{ orgId: FIXTURES.KANSAS_ORG, orgName: "Northside Therapy", role: "admin" }];
      ...
      const ranked = [
        {
          ...
          providerId: FIXTURES.PROVIDER2_ID,
          providerName: "Pat Morgan",
          payerName: "Humana",
          groupName: "Northside Therapy Group",
          state: "KS",
