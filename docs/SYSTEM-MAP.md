# Minted Panel Workbench system map

The detailed orientation that used to live in `CLAUDE.md`. It is no longer
auto-loaded into every session — read it before touching a wire contract, the
panel-mode rules, capture/fill behaviour, or the locked product rules.
`CLAUDE.md` is now a short index that points here.
