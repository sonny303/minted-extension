...
describe("browseableProviders", () => {
  it("drops terminated rows so the picker matches the webapp roster", () => {
    const rows = [
      ...
      provider({ id: "b", firstName: "Casey", lastName: "Reed", status: "terminated" }),
...
describe("providerGroupsLabel", () => {
  ...
  it("joins the groups in the order the server sent (primary first)", () => {
    expect(
      providerGroupsLabel({ groups: [group("Riverside PT", true), group("Acme Health")] }),
    ).toBe("Riverside PT · Acme Health");
  });
...
describe("providerMatchesQuery", () => {
  const withGroups = provider({
    ...
    firstName: "Casey",
    lastName: "Reed",
    groups: [{ id: "g1", name: "Riverside PT", isPrimary: true }],
  });
  ...
  it("narrows two same-named providers by their group — the whole point", () => {
    expect(providerMatchesQuery(withGroups, "casey riverside")).toBe(true);
    expect(providerMatchesQuery(other, "casey riverside")).toBe(false);
  });

  it("matches name, NPI and email case-insensitively", () => {
    expect(providerMatchesQuery(withGroups, "REED")).toBe(true);
  ...
  it("requires EVERY term to hit, so extra words narrow instead of widening", () => {
    expect(providerMatchesQuery(withGroups, "casey nonsense")).toBe(false);
  });
