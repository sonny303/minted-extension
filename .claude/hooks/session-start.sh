...
# each web session. Remote (web) sessions only. Both commands are idempotent
# (exit 0, no-op if already added/installed); failures are swallowed so a
# transient network issue never blocks the session from starting.
set -uo pipefail
